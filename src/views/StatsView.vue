...
async function loadAll() {
    ...
    const [sRes, eRes, rrRes, tRes] = await Promise.all([
        getStats(id, bt), getEquity(id, bt),
        getRRDist(id, bt), getTemporal(bt)
    ])
...
const equityPath = computed(() => {
    ...
    // Drawdown area (entre runMax et valeur courante quand DD < 0)
    let ddArea = ''
    let inDD = false
    let ddPath = ''
    pts.forEach((p, i) => {
...
function barColor(pnl) {
    return parseFloat(pnl) >= 0 ? 'rgba(34,197,94,.7)' : 'rgba(239,68,68,.7)'
}
...
function buildBars(data) {
    ...
    return data.map((d, i) => {
        ...
        return { ...d, x, y, w: bw * 0.8, h, fill: barColor(d.pnl), zeroY, labelX: x + bw * 0.4 }
    })
}
...
function expColor(e) {
    if (e === null) return 'var(--text-2)'
    return parseFloat(e) > 0 ? 'var(--success)' : 'var(--danger)'
}
function pfColor(pf) {
    if (pf === null) return 'var(--text-2)'
    return parseFloat(pf) >= 1.5 ? 'var(--success)' : parseFloat(pf) >= 1.0 ? 'var(--warning)' : 'var(--danger)'
}
</script>
        ...
        <div class="form-group">
          ...
          <div style="display:flex;gap:4px">
            <button v-for="(bt, label) in { Live: false, Backtest: true }" :key="label"
                    ...
                    @click="isBacktest = bt">{{ label }}</button>
          </div>
        </div>
        <div class="form-group">
        ...
        <div v-else style="overflow-x:auto">
          <svg :viewBox="`0 0 ${RR_W} ${RR_H}`" width="100%" :height="RR_H" style="display:block">
            ...
            <g v-for="b in rrChart" :key="b.label">
              ...
              <text :x="b.labelX" :y="b.y - 4" text-anchor="middle" fill="#94a3b8" font-size="10" font-family="Arial">
                {{ b.count }}
              </text>
              <text :x="b.labelX" :y="RR_H - 6" text-anchor="middle" fill="#64748b" font-size="9" font-family="Arial">
                {{ b.label }}
              </text>
            </g>
          </svg>
            ...
            <div style="overflow-x:auto">
              <svg viewBox="0 0 560 140" width="100%" height="140" style="display:block">
                ...
                <g v-for="b in buildBars(temporal.byHour)" :key="b.hour">
                  <rect :x="b.x" :y="b.y" :width="b.w" :height="b.h" :fill="b.fill" rx="2"/>
                  <text :x="b.labelX" :y="TEMP_H - 4" text-anchor="middle" fill="#64748b" font-size="9" font-family="Arial">
                    {{ b.label }}
                  </text>
            ...
            <div style="overflow-x:auto">
              <svg viewBox="0 0 560 140" width="100%" height="140" style="display:block">
                ...
                <g v-for="b in buildBars(temporal.byWeekday)" :key="b.weekday">
                  <rect :x="b.x" :y="b.y" :width="b.w" :height="b.h" :fill="b.fill" rx="2"/>
                  <text :x="b.labelX" :y="TEMP_H - 4" text-anchor="middle" fill="#64748b" font-size="10" font-family="Arial">
                    {{ b.label }}
                  </text>
                  <title>{{ b.label }} | {{ b.count }} trades | {{ b.winrate }}% WR | {{ formatPnl(b.pnl) }}</title>
                </g>
