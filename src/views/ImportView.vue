  ...
  <div>
    ...
    <div class="card" style="margin-bottom:20px">
      ...
      <ol style="color:var(--text-2);font-size:13px;line-height:2;padding-left:20px;margin-top:8px">
        ...
        <li>Seule la section <strong style="color:var(--text)">Trades</strong> est importée — les doublons sont ignorés automatiquement</li>
      </ol>
    </div>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th>Symbole</th><th>Dir.</th><th>Ouverture</th>
              <th>Entrée</th><th>Clôture</th><th>Sortie</th>
              <th>Volume</th><th>P&L</th>
            </tr>
          </thead>
          <tbody>
    ...
    <div v-if="result" class="card">
      ...
      <div v-if="result.skipped > 0"
        ...
        <span style="color:var(--text-muted)">
          (symbole + direction + date + entrée déjà présents en base)
        </span>
      </div>
      <div v-if="result.created === 0 && result.skipped > 0 && !result.errors?.length"
           style="color:var(--text-2);font-size:13px">
        Tous les trades sélectionnés existent déjà dans le journal.
      </div>
      <div v-if="result.errors?.length" style="margin-top:8px">
        ...
        <div v-for="e in result.errors" :key="e"
             style="font-size:12px;color:var(--danger);padding:2px 0">{{ e }}</div>
      </div>
    </div>
  </div>
</template>
