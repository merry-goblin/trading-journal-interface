...
function parseMT5Date(s) {
    return s.trim().replace(/^(\d{4})\.(\d{2})\.(\d{2})/, '$1-$2-$3')
}
...
function toggleRow(i) {
    ...
    if (idx === -1) selected.value.push(i); else selected.value.splice(idx, 1)
}
...
async function handleImport() {
    ...
    try {
        ...
        if (res.data.created > 0 || res.data.completed > 0) { rows.value = []; selected.value = [] }
    } catch (e) {
        result.value = { created: 0, completed: 0, skipped: 0, errors: [e.response?.data?.error ?? 'Erreur serveur'] }
  ...
  <div>
    ...
    <div class="card" style="margin-bottom:20px">
      ...
      <ol style="color:var(--text-2);font-size:13px;line-height:2;padding-left:20px;margin-top:8px">
        ...
        <li>Les positions <strong style="color:var(--text)">incomplètes</strong> (sans clôture en base) sont automatiquement complétées</li>
      </ol>
    </div>
    ...
    <div v-if="result" class="card">
      <div v-if="result.created > 0"
           style="color:var(--success);font-size:15px;font-weight:700;margin-bottom:6px">
        ✓ {{ result.created }} position(s) importée(s).
      </div>
      <div v-if="result.completed > 0"
           style="color:var(--accent);font-size:14px;font-weight:600;margin-bottom:6px">
        🔄 {{ result.completed }} position(s) complétée(s)
        <span style="color:var(--text-muted);font-weight:400;font-size:12px">
          (clôture ajoutée à des positions existantes sans prix de sortie)
        </span>
      </div>
      <div v-if="result.skipped > 0"
           style="color:var(--warning);font-size:13px;margin-bottom:6px">
        ⚠ {{ result.skipped }} doublon(s) ignoré(s)
        <span style="color:var(--text-muted);font-size:11px">(déjà complets en base)</span>
      </div>
      <div v-if="result.created === 0 && result.completed === 0 && result.skipped > 0 && !result.errors?.length"
           style="color:var(--text-2);font-size:13px">
        Tous les trades sélectionnés sont déjà complets dans le journal.
      </div>
